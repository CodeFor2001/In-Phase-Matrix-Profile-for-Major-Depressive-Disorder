...
# aeon imports
from aeon.classification.convolution_based import MultiRocketHydraClassifier  # type: ignore
...
OUT_CSV = OUT_DIR / "mrhydra_results.csv"

# Optional: restrict channels (e.g., 30-channel subset)
...
def to_aeon_panel(X2d):
    ...
    timepoints = 10  # >=9 to satisfy MRHydra kernel size
    X_expanded = np.repeat(X2d[:, :, np.newaxis], timepoints, axis=2).astype(np.float64, copy=False)
    return X_expanded

def evaluate_cv_mrhydra(X, y):
    # X should be 3D: (n_cases, n_channels, n_timepoints)
    skf = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE)
    y_true_all, y_pred_all = [], []

    # MRHydra returns class labels; probability estimates may not be available or stable across versions
    ...
    # AUROC: MRHydra often provides decision_function/predict_proba via underlying linear model.
    # Try safely; if not supported, set to None.
    auc = None
    try:
        # Refit on full data to obtain proba for a fair single estimate could bias; instead aggregate per fold
        # For simplicity, skip AUROC unless predict_proba exists consistently across folds
        pass
    except Exception:
        auc = None
...
def main():
    ...
    for name, X in mats:
        ...
        acc, bacc, auc = evaluate_cv_mrhydra(X_panel, y_clean)
        rows.append({
            "feature_set": name,
        ...
        print(f"{name} [MRHydra]: acc={acc:.3f}, bacc={bacc:.3f}, auc={auc if auc is not None else 'NA'}")
